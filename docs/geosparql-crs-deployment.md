...
The GraphDB GeoSPARQL plugin uses Apache Jena and Apache SIS for coordinate reference system (CRS) operations. CRS84
data works without additional CRS configuration. Projected and other CRSs, including data expressed as
easting/northing coordinates, depend on the Apache SIS CRS definitions and grid files available to GraphDB.

This guide explains how to provide that CRS data and validate it in the target GraphDB installation. For repository
setup and index controls, see [Using the GeoSPARQL plugin](geosparql-usage.md).

## How CRS support works

- The plugin package does not include the Apache SIS EPSG database or national grid-shift files.
- The spatial index uses CRS84 search bounds to find possible matches. GeoSPARQL functions and final relation checks
  use the original geometry literals and their CRSs.
- Missing CRS definitions or grid-shift files are a configuration problem. Coordinates are not silently reinterpreted
  as CRS84 or WGS84.
- A CRS that Apache SIS cannot parse, resolve, or transform fails by default. Missing EPSG or grid data can also reduce
  transformation accuracy without causing an error, so projected CRSs must be checked with known points.
...
- GeoJSON literals use CRS84 and can be indexed and evaluated without extra Apache SIS data.
...
unspecified domains of validity.

Only CRS84 is guaranteed without extra CRS data. Check every projected CRS, other EPSG CRS, national grid, and datum
shift in the target GraphDB runtime before relying on it.
...
For indexed predicates involving different CRSs, the plugin uses CRS84 search bounds to find possible matches and
then verifies each relation using the original geometry literals and their CRSs. The right operand is transformed to
the CRS of the left operand when required.

Mixed-CRS and disjoint queries may examine more candidates than same-CRS queries. Broader candidate searches can
reduce performance, but each possible match is still verified before it is returned. Rebuild the
...
with the available CRS data, indexing fails by default and reports the geometry and CRS. This can happen after an
insert or during a full reindex.
...
The setting applies to subsequent indexing work. After changing it, [rebuild the GeoSPARQL
index](geosparql-usage.md#rebuild-the-index) if existing repository geometries need to be reconsidered. Set the value
to `false` and rebuild to apply strict indexing to all existing geometry data.

`ignoreErrors` does not apply to query-supplied literals. If a SPARQL query supplies a geometry literal with an
unsupported CRS, query evaluation still fails rather than silently treating the coordinates as CRS84 or WGS84.

If the plugin is disabled while data is loaded, unsupported CRS failures are deferred until the plugin is enabled or
the GeoSPARQL index is rebuilt.

## Startup logging

The plugin does not validate arbitrary CRSs at startup and does not fail startup when `SIS_DATA` is missing. Startup
logging reports the Apache SIS data environment visible to GraphDB.
...
The final message is logged as a warning when `SIS_DATA` is configured but unusable. The unset and readable cases are
logged as informational messages.

## Configure CRS data

GraphDB installations that use projected CRSs should provide an Apache SIS data directory and expose it with
`SIS_DATA`.

### Prepare the EPSG database

The plugin includes the Apache Derby runtime, but it does not include the EPSG database. Apache SIS stores its local
EPSG database under `Databases/SpatialMetadata` inside the `SIS_DATA` directory.
...
If your organisation already maintains an Apache SIS data directory, use that instead and validate the CRSs you
need.
...
unless it has been prepared for read-only use and Derby has a writable temporary directory.

### CRS data contents

The required data depends on the CRSs you need to support.
...
- CRSs outside the subset built into Apache SIS, such as `EPSG:3006`, require the external EPSG database.
- British National Grid support such as `EPSG:27700` may require OSGB grid-shift files such as
  `OSTN15_NTv2_OSGBtoETRS.gsb` for high-precision datum transformations.

Ensure that the CRS datasets and grid files used by the installation are licensed for the intended deployment and
distribution model.

## Validate CRS support

The plugin does not run these checks at startup. Test every projected CRS in the target GraphDB runtime before relying
on it.
...
- Run at least one GeoSPARQL function that uses each supported projected CRS.
- Run an indexed GeoSPARQL predicate using each supported projected CRS and, when applicable, a mixed-CRS pair.
...
Run this query against the target GraphDB runtime. It returns `true` when GraphDB loads EPSG:3006 from the external
EPSG database and evaluates the distance in metres:
...
This query checks that GraphDB can load the EPSG:3006 definition and use its metre unit. It does not check a
transformation to another CRS or the accuracy of grid-shift data.

Also test an indexed GeoSPARQL predicate in a temporary repository. Insert the following data, enable GeoSPARQL, and
run the `ASK` query. The expected result is `true`.
...
then GraphDB is running without an explicit Apache SIS data directory. CRS84 data still works, but do not assume that
projected CRSs work until you test them.
...
Do not work around these failures by stripping CRS URIs or relabelling the original coordinates as CRS84. Install the
required CRS data instead. If data is deliberately reprojected before loading, use a trusted transformation and store
the result with the correct CRS URI.
...
- GeoSPARQL predicate and function checks pass.
- An unresolved CRS produces a clear error.
- Known-point checks catch missing data that reduces transformation accuracy.
- The GeoSPARQL index is [rebuilt](geosparql-usage.md#rebuild-the-index) after CRS definitions or grid files change.
